...
public class FragmentDeserializer {
  ...
  public static RelNode deserialize(
      String base64Plan, List<String> fieldNames, List<SqlTypeName> fieldTypes, SchemaPlus rootSchema) {
    String json = new String(Base64.getDecoder().decode(base64Plan), StandardCharsets.UTF_8);

    JavaTypeFactory typeFactory = new JavaTypeFactoryImpl();

    // Use VolcanoPlanner so the deserialized nodes can later be converted to Bindable
