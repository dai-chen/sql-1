...
public class DatetimeUdtExtensionTest extends UnifiedQueryTestBase implements ResultSetAssertion {
    ...
    ResultSet rs =
        planAndExecute(
            "source = catalog.employees"
                ...
                + " | fields last, y");
    // `last` is the return of LAST_DAY(DATE), normalized to standard DATE (not cast to VARCHAR).
    verify(rs)
    ...
    ResultSet rs =
        planAndExecute(
            "source = catalog.employees | where login_time > MAKETIME(8, 0, 0) | fields"
                + " login_time");
    // `login_time` is a bare standard TIME column; with no output cast it stays TIME.
    verify(rs)
  ...
  public void testBareDatetimeColumnPassthrough() throws Exception {
    // Bare DATE/TIME columns pass through unchanged — no normalize match (no UDF), no output cast.
    ResultSet rs = planAndExecute("source = catalog.employees | fields hire_date, login_time");
    verify(rs)
        .expectSchema(col("hire_date", DATE), col("login_time", TIME))
        .expectData(row(java.sql.Date.valueOf("2020-03-15"), java.sql.Time.valueOf("09:30:00")));
  }
