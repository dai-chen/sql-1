...
public class CalcitePlanner implements AutoCloseable {
  ...
  public CalcitePlanner(FrameworkConfig config) {
    ...
    this.validator = new CalciteSqlValidator(opTab, catalogReader, typeFactory,
  ...
  public RelNode convert(SqlNode validated) {
    var converter = new SqlToRelConverter(
        NOOP_EXPANDER, validator, catalogReader, cluster,
        config.getConvertletTable(), converterConfig);
