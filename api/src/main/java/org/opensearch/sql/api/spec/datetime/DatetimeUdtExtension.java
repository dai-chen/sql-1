...
/**
 * Normalizes datetime UDT operations in the logical plan so PPL datetime UDFs and comparisons can
 * accept standard Calcite DATE/TIME/TIMESTAMP inputs from caller-provided schemas in the unified
 * query API.
 *
 * <p>Contributes two ordered post-analysis rules: {@link DatetimeUdtLiteralCoercionRule} casts
 * VARCHAR operands that appear alongside standard datetime operands in non-UDF operators (so {@link
 * DatetimeUdtNormalizeRule} sees homogeneous types); {@link DatetimeUdtNormalizeRule} rewrites UDT
 * calls. The rules share ordering state, so they live in a single extension.
 *
 * <p>Output formatting of datetime columns (e.g., casting DATE results back to ISO strings to match
 * PPL V2's wire contract) is intentionally out of scope here and will be addressed separately for
 * both unified PPL and unified SQL.
 */
public class DatetimeUdtExtension implements LanguageExtension {

  @Override
  public List<PostAnalysisRule> postAnalysisRules() {
    return List.of(new DatetimeUdtLiteralCoercionRule(), new DatetimeUdtNormalizeRule());
  }
