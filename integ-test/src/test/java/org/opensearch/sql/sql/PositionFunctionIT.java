...
public class PositionFunctionIT extends SQLIntegTestCase {
  ...
  public void position_function_test() {
    ...
    JSONObject response =
        executeJdbcRequest(String.format(query, TestsConstants.TEST_INDEX_PEOPLE2));
  ...
  public void position_function_with_function_as_arg_test() {
    String query =
        "SELECT position(upper(str3) IN str1) FROM %s WHERE str1 LIKE 'BINDING SUPPLIES'";
    JSONObject response = executeJdbcRequest(String.format(query, TestsConstants.TEST_INDEX_CALCS));
  ...
  public void position_function_with_null_args_test() {
    ...
    JSONObject response1 =
        executeJdbcRequest(String.format(query1, TestsConstants.TEST_INDEX_CALCS));
    JSONObject response2 =
        executeJdbcRequest(String.format(query2, TestsConstants.TEST_INDEX_CALCS));
