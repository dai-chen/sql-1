...
public class RestUnifiedQueryActionTest {
  ...
  // ---- plan-shape routing gate (isAnalyticsPlanShape / hasRoutableShape) ----
  //
  // This gate decides whether a query gets a second execution path at all, so each shape below is
  // a routing decision, not a style preference. The `top` and `timechart` cases in particular were
  // both silently unrouted at one point: plain `top` because the gate required a non-empty `by`,
  // and `timechart` because it is its own Chart AST node rather than an Aggregation.

  @Test
  public void groupedAggregationIsRoutable() {
    assertTrue(shape("source=logs | stats count() by service"));
    assertTrue(shape("source=logs | stats count(), avg(latency) by service, host"));
  }

  @Test
  public void spanAndBinGroupKeysAreRoutable() {
    assertTrue(shape("source=logs | stats count() by span(ts, 1h)"));
    assertTrue(shape("source=logs | stats count() by span(ts, 1h), service"));
  }

  @Test
  public void timechartIsRoutable() {
    // Chart is a distinct AST node, so the Aggregation branch above never matches it.
    assertTrue(shape("source=logs | timechart span=1h count() by service"));
    assertTrue(shape("source=logs | timechart count() by service"));
  }

  @Test
  public void topAndRareAreRoutableWithOrWithoutByClause() {
    assertTrue(shape("source=logs | top 10 host by service"));
    assertTrue(shape("source=logs | rare 10 host by service"));
    // Plain `top` only collapses to a cheap terms agg when usenull=false; usenull=true leaves a
    // key-ordered composite plus a coordinator-side ROW_NUMBER, which is what we want to route.
    assertTrue(shape("source=logs | top 200 host"));
    assertTrue(shape("source=logs | top 200 usenull=true host"));
  }

  @Test
  public void rawFetchAndUngroupedAggregationAreNotRoutable() {
    assertFalse(shape("source=logs | where status = 500 | fields ts, host | sort ts | head 500"));
    assertFalse(shape("source=logs | stats count()"));
    assertFalse(shape("source=logs | head 10"));
  }

  @Test
  public void unparseableQueryIsNotRoutable() {
    // The gate must not mask a syntax error as a routing decision; the default path re-parses and
    // reports it properly.
    assertFalse(shape("source=logs | this is not ppl"));
    assertFalse(shape(""));
  }

  private boolean shape(String ppl) {
    return action.isAnalyticsPlanShape(ppl, QueryType.PPL);
  }
}
