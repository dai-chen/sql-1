...
| SELECT | select_distinct | SELECT DISTINCT city FROM sanity_test | PASS | ["Denver"], ["Seattle"], ["Portland"] |
...
| GROUP BY | filtered_agg | SELECT city, SUM(age) FILTER(WHERE age > 25) FROM sanity_test GROUP BY city | PASS | ["Seattle", 60], ["Portland", 60], ["Denver", 22] |
...
| CASE | case_in_agg | SELECT city, SUM(CASE WHEN age > 30 THEN 1 ELSE 0 END) AS senior_count FROM s... | PASS | ["Denver", 0], ["Seattle", 1], ["Portland", 1] |
...
| Set Operations | union_basic | SELECT name, age FROM sanity_test WHERE city = 'Seattle' UNION SELECT name, a... | FAIL | HTTP 500: [UNION] is not a valid term at this part of the query: '...RE city = 'Seattle' UNION' <--  |
| Set Operations | union_all_basic | SELECT name, age FROM sanity_test WHERE city = 'Seattle' UNION ALL SELECT nam... | FAIL | HTTP 500: [UNION] is not a valid term at this part of the query: '...RE city = 'Seattle' UNION' <--  |
| Set Operations | minus_basic | SELECT name FROM sanity_test WHERE city = 'Seattle' MINUS SELECT name FROM sa... | FAIL | HTTP 500: [MINUS] is not a valid term at this part of the query: '...RE city = 'Seattle' MINUS' <--  |
...
| DateTime | HOUR | SELECT HOUR(TIME('10:30:00')) | FAIL | HTTP 500: Unable to convert call DATE_PART(string, precision_time<9>?). |
| DateTime | MINUTE | SELECT MINUTE(TIME('10:30:00')) | FAIL | HTTP 500: Unable to convert call DATE_PART(string, precision_time<9>?). |
| DateTime | SECOND | SELECT SECOND(TIME('10:30:45')) | FAIL | HTTP 500: Unable to convert call DATE_PART(string, precision_time<9>?). |
...
| DateTime | TIMESTAMPDIFF | SELECT TIMESTAMPDIFF(DAY, '2024-03-10', '2024-03-15') | FAIL | HTTP 500: Unable to convert call TIMESTAMPDIFF(string, string, string). |
| DateTime | TIMESTAMPADD | SELECT TIMESTAMPADD(DAY, 1, '2024-03-15') | FAIL | HTTP 500: Unable to convert call TIMESTAMPADD(string, i32, string). |
...
| DateTime | CONVERT_TZ | SELECT CONVERT_TZ('2024-03-15 10:00:00', '+00:00', '+05:30') | FAIL | HTTP 500: Unable to convert call convert_tz(string, string, string). |
...
| DateTime | MICROSECOND | SELECT MICROSECOND(TIME('10:30:00.123456')) | FAIL | HTTP 500: Unable to convert call DATE_PART(string, precision_time<9>?). |
...
| Conditional | IFNULL | SELECT IFNULL(NULL, 'default') | FAIL | HTTP 500: Cannot resolve function: IFNULL, arguments: [UNDEFINED,STRING], caused |
