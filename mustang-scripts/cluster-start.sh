...
nohup ./gradlew run -Dsandbox.enabled=true \
    ...
    -Dtests.jvm.argline="-Djava.library.path=$OS_REPO/sandbox/libs/dataformat-native/rust/target/release -Dopensearch.experimental.feature.pluggable.dataformat.enabled=true -Dopensearch.experimental.feature.transport.stream.enabled=true -da" \
    > "$CLUSTER_LOG" 2>&1 &
GRADLE_PID=$!
