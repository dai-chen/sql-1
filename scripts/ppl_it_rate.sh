#!/usr/bin/env bash
# ppl_it_rate.sh — Run Calcite*IT tests with V4 enabled and report pass rate.
# Usage: ./scripts/ppl_it_rate.sh [--class CalciteBasicIT] [--dry-run]
set -euo pipefail
...
CLASS_FILTER="org.opensearch.sql.calcite.remote.Calcite*IT"
DRY_RUN=false
...
for xml in "$XML_DIR"/TEST-org.opensearch.sql.calcite.remote.Calcite*.xml; do
  [[ -f "$xml" ]] || continue
  ...
  p=$((t - f - e - s))
  class=$(basename "$xml" .xml | sed 's/^TEST-org.opensearch.sql.calcite.remote.//')
  CLASS_BREAKDOWN="${CLASS_BREAKDOWN}${class}|${t}|${p}|$((f + e))|${s}\n"
  ...
  if [[ $((f + e)) -gt 0 ]]; then
    fqclass=$(basename "$xml" .xml | sed 's/^TEST-//')
...
# Per-class breakdown (sorted by failures descending)
echo ""
echo "Per-class breakdown (classes with failures):"
echo "  Class | Total | Passed | Failed | Skipped"
echo "  ------|-------|--------|--------|--------"
echo -e "$CLASS_BREAKDOWN" | sort -t'|' -k3 -nr | while IFS='|' read cls tot pas fai ski; do
  [[ -z "$cls" ]] && continue
  [[ "$fai" -eq 0 ]] && continue
  printf "  %-50s %4s %6s %6s %6s\n" "$cls" "$tot" "$pas" "$fai" "$ski"
done

echo ""
echo "Fully passing classes:"
echo -e "$CLASS_BREAKDOWN" | sort -t'|' -k1 | while IFS='|' read cls tot pas fai ski; do
  [[ -z "$cls" ]] && continue
  [[ "$fai" -ne 0 ]] && continue
  printf "  %-50s (%s tests)\n" "$cls" "$tot"
done
...
if [[ -n "$FAIL_LIST" ]]; then
  echo ""
  echo "Failing tests:"
  echo "$FAIL_LIST" | head -80
  if [[ $(echo "$FAIL_LIST" | wc -l) -gt 80 ]]; then
...
# Write per-class breakdown
echo "Class|Total|Passed|Failed|Skipped" > "$REPORT_DIR/per-class.csv"
echo -e "$CLASS_BREAKDOWN" | sort -t'|' -k3 -nr >> "$REPORT_DIR/per-class.csv"
...
echo "  per-class.csv  — per-class breakdown"
