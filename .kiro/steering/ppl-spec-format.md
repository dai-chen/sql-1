...
1. **Read Splunk Documentation:**
   ...
   - Read the entire page end-to-end
...
1. ❌ Skipping Splunk baseline study before writing requirements
2. ❌ Having more than 3 requirements in requirements.md
3. ❌ Missing query example with input/output for each requirement
4. ❌ Missing expected output tables in requirements.md
5. ❌ Not answering all 4 high-level decisions in design.md
6. ❌ Missing SQL equivalents for each use case
7. ❌ Not specifying pushdown feasibility with reasoning
8. ❌ Not listing UDF/UDAF requirements explicitly
9. ❌ Generic descriptions instead of concrete examples

## When Generating Specs

1. **Always start** with Splunk baseline study (read Splunk docs first)
2. **Document baseline** in requirements.md before requirements
3. **Write at most 3 requirements** with ONE query example each (input + output)
4. **Always include** the 4 high-level decisions in design.md
5. **Always show** SQL equivalents for each requirement's query example
6. **Always specify** pushdown feasibility with reasoning
7. **Always list** required UDFs with signatures
